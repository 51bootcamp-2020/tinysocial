...
const write_fake_db = async () => {
    const db = await createStore(true);
    ...
    const read_info = [
        ...
        {csv: 'UserParticipatedEvent.csv', dat: userParticipatedEvents, db: db.UserParticipatedEvent},
    ];
    ...
    for(let j = 0; j < read_info.length; j++){
        ...
        let end = new Promise(function(resolve, reject) {
            fd.on('data', async (data) => await read_info[j].dat.push(data)).on('end', async () => {
                /*
