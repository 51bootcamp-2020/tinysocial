...
describe('[Mutation]', () => {
  test('signInWithGoogle', async () => {
    signInWithGoogle.mockReturnValueOnce(
        {success: true, message: 'Success', token: 'TOKEN', user: {id: 1}});
    const res = await Mutation.signInWithGoogle({}, {googleId: 'test@test.com'},
        mockContext);
    expect(res).
        toEqual(
            {success: true, message: 'Success', token: 'TOKEN', user: {id: 1}});
  ...
  test('signUpWithGoogle', async () => {
    signUpWithGoogle.mockReturnValueOnce(
        {success: true, message: 'Success', token: 'TOKEN', user: {id: 1}});
    const res = await Mutation.signUpWithGoogle({}, {googleId: 'test@test.com'},
        mockContext);
    expect(res).
        toEqual(
            {success: true, message: 'Success', token: 'TOKEN', user: {id: 1}});
  ...
  test('signIn', async () => {
    ...
    const res = await Mutation.signIn({},
        {email: 'test@test.com', pw: 'testpw'}, mockContext);
    expect(res).
        toEqual(
            {success: true, message: 'Success', token: 'TOKEN', user: {id: 1}});
  });
  ...
  test('logOut', async () => {
    expect(Mutation.logOut({}, {}, mockContext)).
        rejects.
  ...
  test('createOrModifyOfReview', async () => {
    ...
    const res = await Mutation.createOrModifyReview({},
        {eventId: 1, title: 'testTitle', content: 'testContent', isPublic: true},
        mockContext);
